...
.try
:LDX #0:STY tempy:.trynext:INX:CPX StrAlen:BNE vc:TXA:RTS:.vc:INY:LDA(APtr),Y:CMP StrA%,X
...
.isn
{
I	NC sno:BNE pb:INC sno+1:.pb:RTS
}
...
.DFS
JSR him
.dfs:LDX #0:STX comprec:STX e+1:JSR ifp:JSR ims:LDY #0:STY tempy
.ma:JSR sl:JSR pre:JSR printselection:JSR nxr:LDY #0:LDA(APtr),Y:BNE ma:JSR cursorset:
...
:LDY #Btmw%:JSR slw:JSR icd:\init catdat
JSR ifr:
JSR tfa:\terminate filter array
.xa:INC catdat+6:JSR cce:CMP #1:BEQ xb:\check catdatx exists
...
.fbd
 ...
 AND #3:CMP x+1:BNE dn:JSR crd:.dn:JSR nxr:JMP fbd:.oa:JSR ssfr
.xc:LDA e:AND #2:BEQ xd

\Filter by pub fbp
.fbp
LDY #0:LDA(APtr),Y:BEQ ob::JSRged:LDA(APtr),Y:CMP y:BNE du:INY:INY:INY:LDA(APtr),Y
AND #3:CMP y+1:BNE du:JSR crd:.du:JSR nxr:JMP fbp:.ob:JSR ssfr
.xd:LDA e:AND #4:BEQ xe

\Filter by type fbt
.fbt
LDY #0:LDA(APtr),Y:BEQ oc::JSR ged:INY:INY:INY:LDA(APtr),Y
AND #&1C:CMP v:BNE dy:JSR crd:.dy:JSR nxr:JMP fbt:.oc:JSR ssfr
...
.cursordown:
{
 INC cnl:JSR cursorclear:CLC:LDA #&28:ADC curadd:STA curadd:LDA #0:ADC curadd+1:STA curadd+1:JMP cursorset
}\rts
.cursorup
{
...
AND #3:STA z+1:LDA(APtr),Y:ROR A:ROR A:AND #&F:TAY:LDA type,Y:JSR osasci:LDA #134:JSR osasci
\have pub in z
...
CMP #&80:BCS os:JSR osasci:INC cll:LDA cll:CMP #pll%:BCS na:INY:BNE ot:.os:AND #&7F:JSR osasci
.na:RTS
.type:EQUS"ACGSMPUZ"
\Convert Case cvc
.cvc
