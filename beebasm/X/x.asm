...
osrdch=&FFE0:oscli=&FFF7:osfsc=&21E:t=&FFF1
...
\have diskname
	...
	BVC ready:\jmp 
	.zc
	\do not have diskname so need to get current disk
	JSR gettitleopt
{
	LDX #0
	...
	\*rep2
{
	...
	\set working drive  to 0
	LDX #'0'
	STX requesteddrive
	JSR setdrivex
	\issue "din" command for drive 0
	...
	\display text
	LDA #rep3%:STA tempx:JSR reptoninstructions
	...
	\set to dr.0
{
	...
	JSR setdisk
	\delete g.a file (if exists
	LDA #LO(preludetxt):STA blockstart:
	LDA #HI(preludetxt):STA blockstart+1:
	LDX #blockstart:LDY #0:LDA #6:JSR osfile:
	...
	\rename file to g.a
	LDX #renamecmd%
	JSR initprepcmd:JSR addpram:LDX strAoffset:INX:STX strAoffset
...
.screencheck
\we 7F00 to 7F07 is direct screen load
{
	CMP #8:BCS notcoded
...
\&FE30
	.romcheck
	LDA load+1:CMP #&80:BNE prepload
{
	\have a rom need to ask what bank
	
	\have loaded into 1100
	LDX #romtext%:JSR diserror
	LDA #14:STA strA%
	LDX #LO(txtip):LDY #HI(txtip):LDA #0:JSR osword
	STY strAlen:LDA strA%:BNE bb
	\default
	LDA #14:BNE bc
	.bb	
	LDA strA%:SEC:SBC #48:CMP #11:BCC bc
	\have a letter
	SEC:SBC #7:
	.bc:\now have required bank
	\no's need reversing 15=0 14=1 etc
	\AUG pg 195
	EOR #&FF
	AND #&F
	STA tempy
	...
	INY
	BNE mvmem
	INC mvmem+2:INC mvmem+6
	DEX:BNE mvmem
	LDY tempy
	STY romno:JMP osrdrm:\RTS
	RTS
}
\prepload
.prepload
{
	...
	.zeropage
	LDY #codeend-codebegin:.av:LDA codebegin,Y:STA codestart,Y:
	DEY:BPL av
	\CODE NOW IN ZERO PAGE
	\prep for oscli call
	LDY #HI(strA%):LDX #LO(strA%)
	JMP codestart:\into zero page
	\-----------------------
			.codebegin
			...
			.iq1:LDA(loadadd),Y:STA(trueadd),Y:INY:BNE iq1
			INC loadadd+1:INC trueadd+1:DEX:BPL iq1
			\JUMP OR RTS
...
{
	LDY #&FF:LDX #0
	.mq:INY:LDA strA%,Y:STA strB%,X:INX:CMP #&D:BNE mq:RTS
}
\copy from strB% to pram
.StrBtopram
{
	LDY #&FF:LDX #0
	.mq:INY:LDA strB%,Y:STA pram%,X:INX:CMP #&D:BNE mq:RTS
}
\execmd
.execmd
{
...
{
	LDY #0
	.ez:DEX:BNE nexcmd:LDX strAoffset:.ey:LDA cmdadd,Y
	CMP #&80:BCC am:AND #&7F
...
.getfilesystemtype
{
	...
	LDA #0:STA tempx
	.cc:INC tempx:LDX tempx:JSR initprepcmd
...
\copyargumentXintostrA takes X rets strA%
.copyargumentXintostrA
{
	...
	\good to copy!
	\ X=0!
	.ad:LDA(osargsptr),Y:STA strA%,X:CMP #32:BEQ ae:CMP #&D:BEQ af
	INX:INY:BNE ad
	.ae
	LDA #&D:STA strA%,X
	.af
...
\#10
romtext%=10
EQUS "ROM detected default bank E",&D
EQUS "ensure set to Write",&D
EQUS"Enter bank no in HEX (E)",&8D
...
\text entry setup for roms
.txtip
EQUB LO(strA%):EQUB HI(strA%):EQUB 1:EQUB 48:EQUB 70
