...
.ldin
{
JSR ifcp:LDA #0:STA comprec:LDA dinadd:STA block:LDA dinadd+1:STA block+1:JMP lf
}\rts
\Loadfile lf†Needs Aptr set
.lf
{
LDA APtr:STA block+2:LDA APtr+1:STA block+3:LDA #0:STA block+6:LDA #&FF:LDX blockadd:LDY blockadd+1:JMP osfile
}\rts
...
LDA #LO(prtt):STA APtr:JSR cad:JMP initreccount
}\rts
\HardInitmenu him
.him
...
LDX#0:STX comprec:.ms:LDY #0:LDA(APtr),Y:BEQ itm:JSR cas:JSR esa:LDY #0:.mu:LDA StrA%,Y:STA FS%,X:CMP #&80:bCS mv:INY:INX:BNE mu:.mv:JSR nxr:INX:CLC:BCC ms
\Initmenu itm  
.itm
{
