...
\.din
\{
\LDX dsadd:LDY dsadd+1:JMP oscli
\}\RET
\.dsadd
...
\.ds
\{
\STA drstr+3:LDX drstradd:LDY drstradd+1:JMP oscli
\} \rts
...
\STX StrA%
\LDX #48:.lh:LDA IntA:SEC:SBC #10:BCC lj:INX:STA IntA:BNE lh:.lj
\STX dinstr+4:STX StrA%+1:LDA #48:ADC IntA:STA dinstr+5:STA StrA%+2:RTS 
\}
\check catfile exists cce
.cce
{
LDA #LO(catdat):STA block:LDA #HI(catdat):STA block+1:LDA #5:LDX #LO(block):LDY #HI(block):JMP osfile:
}\rts
\load catfile lcf
.lcf:
{
JSR ifcp:LDA #4:STA comprec:LDA #LO(catdat):STA block:LDA #HI(catdat):STA block+1:JMP lf:
}\rts
\Loadpub lpub
.lpub
{
JSR ifsp:LDA #0:STA comprec:LDA #LO(pubrec):STA block:LDA #HI(pubrec):STA block+1:JMP lf:
} \rts
\Loaddin ldin
.ldin
{
JSR ifcp:LDA #0:STA comprec:LDA #LO(dinrec):STA block:LDA #HI(dinrec):STA block+1:JMP lf
}\rts
\Loadfile lf†Needs Aptr set
.lf
{
LDA APtr:STA block+2:LDA APtr+1:STA block+3:LDA #0:STA block+6:LDA #&FF:LDX #LO(block):LDY #HI(block):JMP osfile
}\rts
...
.det
{
LDY #Btmw%:JSR slw:LDA #131:JSR osasci:JSR psx:LDA #Clet%:JMP osasci
}\rts
...
\favorate
.tlf
:LDX #0:LDA w:EOR #&40:STA w:BNE jy:LDX #0:.jx:INX:LDA fav,X:CMP #&80:BCC jx:INX:.jy:LDA e:EOR #8:STA e:LDY #0:.jw:LDA fav,X:STA StrA%,Y:INY:INX:CMP #&80:BCC jw:DEY:JSR esa:JSR itfs:JSR clz:LDA #1:STA z
JSR mrz:JSR copystrToArray:JSR itm:JMP dfs
\Din by no
\JSR gnz:LDY #&5C:JSR ciz:JSR ldin:JSR mrz:LDA #0:STA c:JSR wtf:JSR wdn:JMP dfs
.bbb:CMP #67:BNE ccc
\publisher
JSR lpub:LDA #&60:STA b:LDA #2:STA c:JSR gsc:JSR wtf:LDA e:ORA #2:STA e:JMP dfs
.ccc:CMP #68:BNE ddd
\progtype
...
\Search din
LDX #0:STX c:STX tempx:JSR det:JSR ldin:JSR gss:LDA fr%:BNE oy:JSR nsr:JMP dfs:.oy:LDY #&5C:JSR czi:JSR wtf:
\JSR wdn:
JMP dfs
...
\JSR ddc:LDY #0:.ae:LDA boot,Y:STA StrA%,Y:INY:CPY #8:BNE ae:LDX #0:.ag:LDA dinstr+3,X:STA StrA%,Y:INY:INX:CPX \#3:BNE ag:RTS:.boot:EQUS"X !BOOT "
.mmm:CMP #78:BNE nnn:

\Launch help
.hlp:STA hlpcmd
.straighthelp:LDX #LO(hlpcmd):LDY #HI(hlpcmd):JMP oscli:\RTS
.hlpadd:EQUW hlpcmd:.hlpcmd:EQUS"N":EQUB &D
.nnn:CMP #63:BNE ooo
\? pressed
BEQ straighthelp
.ooo:RTS
...
.nsr
{
LDX #0:STX comprec:STX e+1:LDA #LO(err):STA APtr:LDA #HI(err):STA APtr+1:JSR gns:JMP getinput:}\rts
\apply filters afs
.applyfilters
.afs
\are any filters set?
...
STA stxt%,Y:DEY:BPL ak:LDA #5:STA c:LDA e:ORA #&10:STA e:LDY StrAlen
...
\dr/din/cat ddc
\this needs to be changed todo
\.ddc
\{
\LDA #51:JSR ds:LDY #&5C:JSR ciz:JSR sdz:LDY #2:.og:LDA StrA%,Y:STA dinstr+3,Y:DEY:BPL og:JMP din
\} \RET
\PrintString X psx 
.psx
{
LDX tempx:LDY #0:.ju:CPX #0:BEQ jv:.jw:INY:LDA ptxt,Y:CMP #&80:BCC jw:DEX:BNE ju
INY:.jv:LDA ptxt,Y:AND #&7F:JSR osasci:LDA ptxt,Y:INY:CMP #&80:BCC jv:RTS 
}

\get no in z gnz maX len
\.gnz:
\{
\JSR clz:LDX #1:STX tempx:JSR det:JSR esn:STY tempy:LDY #0:.lm:LDA StrA%,Y:SEC
\SBC #48:STA z:STA tempx:INY:TYA:CMP tempy:BEQ iq:.io:JSR mbt:LDA StrA%,Y
\SEC:SBC #48:CLC:ADC z:STA z:STA tempx:LDA #0:ADC z+1:STA z+1:INY:TYA:CMP tempy:BNE io:.iq
\LDA z+1:CMP #1:BCC ir:BEQ is:BCS gnz:.is:LDA z:CMP #&FF:BCS gnz:.ir:LDY #&5C:JSR czi:LDY tempy:DEY:RTS
\}
\write din no wdn
\.wdn
\{
\LDY #&5C:JSR ciz:LDY #0:JSR sdz:LDY #2:JSR esa:JSR itfs:JSR clz:LDA #1:STA z:JSR mrz
\JSR copystr0ToArray:JSR itm:LDA e:ORA #1:STA e:
RTS
\}
\Multiply by 10 mbt
\.mbt
...
:JSR cas:.lwtf
{
JSR esa:JSR itfs:JSR clz:LDA c:STA z:JSR mrz:JSR copystrToArray:JMP itm
}\rts
...
.cursordown:
{
 INC cnl:JSR cursorclear:CLC:LDA #&28:ADCcuradd:STA curadd:LDA #0:ADC curadd+1:STA curadd+1:JMP cursorset
}\rts
.cursorup
{
DEC cnl:JSR cursorclear:SEC:LDA curadd:SBC #&28:STA curadd:LDA curadd+1:SBC #0:STA curadd+1:JMP cursorset
}\rts
}
.cursorset:
{
...
\have pub in z
LDA #HI(fl%):STA Apub+1:LDA #LO(fl%):STA Apub:.on:LDA z+1:BEQ op:DEC z+1:LDX #0:.oq:LDY #0:.ou:LDA(Apub),Y:CMP #&80:BCS or:INY:BNE ou:.or:INY:CLC:TYA:ADC Apub:STA Apub:LDA #0:ADC Apub+1:STA Apub+1:DEX:BNE oq:BEQ on
...
\.numadd:EQUW numip
.numip:EQUB LO(StrA%):EQUB HI(StrA%):EQUB 3:EQUB 48:EQUB 57}
...
.iptt
{
LDA #HI(prtt):STA APtr+1: LDA #LO(prtt):STA APtr:JSR cad:JMP initreccount
}\rts
\HardInitmenu him
.him
{
JSR lpub:.caz:LDY #atozlen%:LDA#0:.zy:STA atozStart%,Y:DEY:BPL zy
LDA #HI(selectiontext):STA APtr+1:LDA #LO(selectiontext):STA APtr
LDX#0:STX comprec:.ms:LDY #0:LDA(APtr),Y:BEQ itm:JSR cas:JSR esa:LDY #0
.mu:LDA StrA%,Y:STA FS%,X:CMP #&80:bCS mv:INY:INX:BNE mu:.mv:JSR nxr:INX:CLC:BCC ms
}
\Initmenu itm  
.itm
{
LDA #HI(cinit%):STA curadd+1:LDA #LO(cinit%):STA curadd:RTS
}
